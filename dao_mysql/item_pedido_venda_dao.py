"""
DAO para manipulação da tabela Item_Pedido_Venda no MySQL
"""
    ...
    def atualizar(self, id_item_pedido_venda: int, quantidade: int = None,
        ...
        try:
            ...
            if preco_unitario_venda is not None:
                campos.append("preco_unitario_venda = %s")
        ...
        try:
            with get_cursor(commit=False) as cursor:
                cursor.execute("""
                    SELECT COUNT(*) as total
                    FROM Item_Pedido_Venda
                    WHERE id_pedido_venda = %s AND id_produto = %s
        ...
        try:
            with get_cursor(commit=False) as cursor:
                cursor.execute("""
                    ...
                    FROM Item_Pedido_Venda ipv
                    JOIN Produto p ON ipv.id_produto = p.id_produto
                    WHERE ipv.id_pedido_venda = %s AND ipv.id_produto = %s
                """, (id_pedido_venda, id_produto))
        ...
        try:
            with get_cursor(commit=False) as cursor:
                cursor.execute("""
                    ...
                    FROM Item_Pedido_Venda ipv
                    JOIN Produto p ON ipv.id_produto = p.id_produto
                    WHERE ipv.id_pedido_venda = %s AND p.estoque_atual < ipv.quantidade
                """, (id_pedido_venda,))
