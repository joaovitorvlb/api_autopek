...
class NivelAcessoDAO:
    """
    DAO para operações na tabela nivel_acesso.
    ...
    def inserir(self, nome):
        ...
        with get_cursor() as cur:
            cur.execute(
                "INSERT INTO nivel_acesso (nome) VALUES (%s)",
                (nome,)
            )
    ...
    def atualizar(self, id_nivel_acesso, nome):
        ...
        with get_cursor() as cur:
            cur.execute(
                "UPDATE nivel_acesso SET nome = %s WHERE id_nivel_acesso = %s",
    ...
    def deletar(self, id_nivel_acesso):
        ...
        """
        with get_cursor() as cur:
            cur.execute("DELETE FROM nivel_acesso WHERE id_nivel_acesso = %s", (id_nivel_acesso,))
    ...
    def contar_usuarios(self, id_nivel_acesso):
        ...
        with get_cursor() as cur:
            cur.execute(
                "SELECT COUNT(*) as count FROM usuario WHERE id_nivel_acesso = %s",
                (id_nivel_acesso,)
            )
            result = cur.fetchone()
            return result['count']
    ...
    def verificar_nome_existe(self, nome, id_nivel_acesso_excluir=None):
        ...
        with get_cursor() as cur:
            if id_nivel_acesso_excluir:
                cur.execute(
                    "SELECT COUNT(*) as count FROM nivel_acesso WHERE nome = %s AND id_nivel_acesso != %s",
                    (nome, id_nivel_acesso_excluir)
                )
            else:
                cur.execute(
                    "SELECT COUNT(*) as count FROM nivel_acesso WHERE nome = %s",
                    (nome,)
                )
            result = cur.fetchone()
